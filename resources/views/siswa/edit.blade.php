    ...
    <div class="row" id="edit-kelas">
        <div class="demo-vertical-spacing demo-only-element col-md-12 ">
            <div class="form-password-toggle">
                ...
                <input type="text" value="{{ $siswa->id }}" name="id" hidden>
                <input type="number" class="form-control" id="nisn" name="nisn"
                    aria-describedby="basic-default-password2" required value="{{ $siswa->nisn }}" />
                <span class="invalid-feedback alert-nisn d-none" role="alert"></span>
            </div>
            <div class="form-password-toggle">
                <label class="form-label" for="nama">Nama</label>
                <input type="text" class="form-control" id="nama" name="nama"
                    aria-describedby="basic-default-password2" required value="{{ $siswa->nama }}" />
            ...
            <div class="form-password-toggle">
                ...
                <input type="date" class="form-control" id="tanggal_lahir" name="tanggal_lahir"
                    aria-describedby="basic-default-password2" required value="{{ $siswa->tanggal_lahir }}" />
                ...
                <select class="form-select" id="kelas_id" required name="kelas_id">
                    @foreach ($kelas as $k)
                        @if ($siswa->kelas_id == $k->id)
                            <option value="{{ $k->id }}" selected>{{ $k->kelas }}
                            </option>
                        @endif
...
<div class="modal-footer">
    ...
    <button class="btn btn-primary" onclick="update({{ $siswa->id }})">Save
        changes</button>
</div>
